...
int board[1003][1003]; 
int tot_area[103]; //차지하는 총 면적
int n,sm,sh; //n:횟수, sm:밑변, sh:높이
 
typedef struct square{ //색종이의 왼쪽아래, 오른쪽위 좌표
    int lx; int ly; 
    int rx; int ry;
}SQUARE;
...
int main(){    
    ...
    for(int i = 0; i <= 101; i++){ //board 초기화
        for(int j = 0; j <= 101; j++){
            board[i][j] = -1;
        }
    }
    
    for(int i = 0; i < n; i++){ //왼쪽 아래 좌표 입력, 밑변 및 높이로 오른쪽 위 좌표계산
        cin >> s[i].lx >> s[i].ly >> sm >> sh; 
        s[i].ry = s[i].ly + sh;
        s[i].rx = s[i].lx + sm;    
    }
    
 
    for(int i = 0; i < n; i++){ //색종이 입력 순서대로 면적만큼 표시
        for(int j = s[i].ly; j < s[i].ry; j++){
            for(int k = s[i].lx; k < s[i].rx; k++){
                board[j][k] = i;
            }
        }
    }
 
    for(int k = 0; k < n; k++){ //표시된 board에서 색 
        for(int i = 0; i <= 101; i++){
            for(int j = 0; j <= 101; j++){
                if(board[i][j] == k) tot_area[k] += 1;
            }
        }
    }
 
    for(int i = 0; i < n; i++){
        cout << tot_area[i] << endl;
    }
